...
class ColBERT(BaseColBERT):
    ...
    def doc(
        ...
        sparsity_scores = sparsity_scores.view(-1, sparsity_scores.size(-1)).unsqueeze(
            -1
        )
    ...
    def output_stats(self):
        ...
        if len(self.keep_stats) > 0:
            ...
            torch.save(
                keep_stats,
                os.path.join(save_dir_path, f"{param_str}_keep_mask_stats_keep.pt"),
            )
            torch.save(
                discard_stats,
                os.path.join(save_dir_path, f"{param_str}_keep_mask_stats_discard.pt"),
            )
