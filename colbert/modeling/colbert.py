...
class ColBERT(BaseColBERT):
    ...
    def doc(
        self, input_ids, attention_mask, keep_dims=True, include_sparsity_scores=True
    ):
        ...
        D = self.linear(D)
        sparsity_scores = self.slinear(D)  # Shape B x num_docs x 1

        # Zero-extend last dimenstion of sparsity coefficients
        zeros_fill = torch.zeros(D.size(0), D.size(1), 1).to(self.device)
        sparsity_scores_ext = torch.cat((sparsity_scores, zeros_fill), dim=-1)

        # TODO: test output from sigmoid vs softmax from sparse layer

        # Apply sigmoid (for training, indexing)
        sparsity_scores = self.ssigmoid(sparsity_scores)

        # Compute gumbel_softmax scores for the last dimension
        gumbel_sparsity_scores = torch.nn.functional.gumbel_softmax(
            sparsity_scores_ext, tau=1, hard=True  # Q: True?
        )

        # Get rid of the redundant layer
        gumbel_sparsity_scores = gumbel_sparsity_scores[:, :, 0].unsqueeze(-1)

        # TODO: Debug print these masks ^
        ...
        rhat_shape = rhat.shape  # Shape: B x numdocs x 128
        gumbel_sparsity_scores_shape = gumbel_sparsity_scores.shape

        rhat = rhat.view(-1, rhat.size(-1)).unsqueeze(
            -1
        )  # Shape: (B * numdocs) x 128 x 1
        gumbel_sparsity_scores = (
            gumbel_sparsity_scores.view(  # Shape: (B * numdocs) x 1 x 1
                -1, gumbel_sparsity_scores.size(-1)
            ).unsqueeze(-1)
        )

        # TODO: Rework this to masking

        r = torch.bmm(rhat, gumbel_sparsity_scores)
        r = r.view(rhat_shape)
        if self.use_gpu:
            r = r.half()
        ...
        if self.colbert_config.sparse_reduce and keep_dims in [
            ...
            if sparse_reduce_type == "threshold":
                ...
                self.sparsity_stats.append(sparsity_scores[mask.bool()])
    ...
    def output_stats(self):
        ...
        if len(self.keep_stats) > 0:
            ...
            sparsity_stats = torch.cat(self.sparsity_stats, dim=0)
            ...
            if self.colbert_config.sparse_reduce_type == "threshold":
                ...
                param_str = (
                    f"quantile-maxnorm-{self.colbert_config.sparse_reduce_quantile}"
                )
